...
window.BORep = (function () {
  ...
  function showPreview(a) {
    srv('reportData', a).then(function (d) {
      var cols = d.columns || [];
      /* THE META IS WHERE THE HONESTY LIVES. The Profit report puts "these units had no cost
         price recorded, so this figure is the HIGHEST it could be" in d.meta; the PDF and the
         spreadsheet both print it and the Preview -- the one-tap way to look at this on a phone
         -- dropped it, showing an owner a 100% margin as fact. Anything in meta that is a
         warning is shown as one; the rest is the ordinary "generated by / generated at" run of
         lines and stays quiet. */
      var notes = (d.meta || []).filter(function (m) { return !/^Generated/.test(m); });
      var h = '<div class="small muted" style="margin-bottom:8px;">' + esc(d.subtitle || '') + '</div>'
        + notes.map(function (m) {
            return '<div class="' + (/WARNING/i.test(m) ? 'alert-warning' : 'small muted') + '" style="margin-bottom:8px;">' + esc(m) + '</div>';
          }).join('')
        + '<div class="table-wrap"><table class="bo-table"><thead><tr>' + cols.map(function (c) { return '<th' + (c.align === 'right' ? ' class="right"' : '') + '>' + esc(c.label) + '</th>'; }).join('') + '</tr></thead><tbody>';
