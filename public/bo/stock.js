...
window.BOStock = (function () {
  ...
  function listUnits() {
    ...
    srv('units', args).then(function (r) {
      ...
      rows.forEach(function (u) { var st = u.status === 'in_stock' ? 'badge-ok' : u.status === 'sold' ? 'badge-seller' : u.status === 'lost' ? 'badge-out' : 'badge-low'; h += '<tr><td class="mono">' + esc(u.imei || '–') + '</td><td class="mono small">' + esc(u.serial_no || '–') + '</td><td>' + esc(u.product_name || '') + '</td><td class="small">' + esc(shopName(u.branch_id)) + '</td><td><span class="badge ' + st + '">' + esc(u.status) + '</span></td><td class="small">' + BO.fmtDate(u.received_at) + '</td><td style="white-space:nowrap;"><button class="btn-sm-primary" onclick="BOStock.history(\'' + BO.jsq(u.id) + '\')">History</button> ' + (u.status !== 'sold' ? '<button class="btn-sm-warning" onclick="BOStock.editUnit(\'' + BO.jsq(u.id) + '\',\'' + BO.jsq(u.imei || '') + '\',\'' + BO.jsq(u.serial_no || '') + '\',\'' + BO.jsq(u.branch_id || '') + '\',\'' + BO.jsq(u.status) + '\')">Edit</button>' : '') + '</td></tr>'; });
  ...
  function editUnit(id, imei, serial, branch, status) {
    BO.dialog({ title: 'Edit unit', body: '<div class="form-group" style="margin-bottom:10px;"><label class="form-label">IMEI</label><input class="form-control mono" id="euImei" value="' + esc(imei) + '"></div><div class="form-group" style="margin-bottom:10px;"><label class="form-label">Serial</label><input class="form-control" id="euSerial" value="' + esc(serial) + '"></div>' + (S.branches.length ? '<div class="form-group" style="margin-bottom:10px;"><label class="form-label">Shop</label><select class="form-select" id="euBranch">' + shopOpts(branch, '— No shop —') + '</select></div>' : '') + '<div class="form-group" style="margin-bottom:10px;"><label class="form-label">Status</label><select class="form-select" id="euStatus"><option value="in_stock"' + (status === 'in_stock' ? ' selected' : '') + '>In stock</option><option value="lost"' + (status === 'lost' ? ' selected' : '') + '>Lost</option>' + (status === 'lent' ? '<option value="lent" selected>Lent (returned via Lendings)</option>' : '')
      /* A HELD HANDSET HAD NO OPTION OF ITS OWN, so the browser pre-selected "In stock" and
         saving an unrelated edit -- a corrected IMEI -- silently released somebody's phone.
         The server refuses it now; the dialog must not offer it either. */
      + (status === 'reserved' ? '<option value="reserved" selected>Held for a customer (release it on Holds)</option>' : '') + '</select></div><div class="form-group"><label class="form-label">Note</label><input class="form-control" id="euNote" placeholder="why it changed"></div>',
      footer: '<button class="btn-secondary" onclick="BO.closeDialog()">Cancel</button><button class="btn-primary" onclick="BOStock.saveUnit(\'' + BO.jsq(id) + '\')">Save</button>' });
  }
