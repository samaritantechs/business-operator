...
window.BODash = (function () {
  ...
  function chips() {
    ...
    S.branches.forEach(function (b) { h += '<button class="tab-chip' + (branchId === b.id ? ' active' : '') + '" onclick="BODash.branch(\'' + BO.jsq(b.id) + '\')">' + esc(b.name) + '</button>'; });
    return h + '</div>';
  }
  ...
  function recent() {
    ...
    srv('recentSales', { limit: 30, include_cancelled: true, branch_id: branchId || undefined }).then(function (r) {
      ...
      rows.forEach(function (s) {
        ...
        var pay = '<span class="badge ' + BO.badgeFor(s.payment_method) + '">' + esc(s.payment_method) + '</span>' + (s.partner_name ? '<div class="small muted">' + esc(s.partner_name) + ' · <a href="#" class="link-btn" onclick="BODash.partnerPaid(\'' + BO.jsq(s.id) + '\',' + (s.partner_paid ? 'false' : 'true') + ');return false;">' + (s.partner_paid ? '✔ Paid' : 'Unpaid') + '</a></div>' : '');
        h += '<tr style="' + (cancelled ? 'opacity:.55;' : '') + '"><td class="mono small muted">' + esc(s.legacy_id || '') + '</td><td class="small" style="white-space:nowrap;">' + BO.fmtDT(s.sold_at) + '</td><td>' + esc(s.seller_name) + '</td><td>' + esc(s.product_name) + (s.imei ? '<div class="mono small muted">' + esc(s.imei) + '</div>' : '') + ((s.brand || s.model) ? '<div class="small muted">' + esc([s.brand, s.model].filter(Boolean).join(' ')) + '</div>' : '') + '</td><td>' + s.qty + '</td><td>' + unit + '</td><td class="mono">' + fmtFull(s.total) + '</td><td>' + pay + '</td><td class="small">' + esc(s.branch_name || '') + '</td><td>' + (cancelled ? '<span class="badge badge-cancel">Cancelled</span><div class="small muted">' + esc(s.cancelled_by_name || '') + (s.cancel_reason ? ': ' + esc(s.cancel_reason) : '') + '</div>' : '<span class="badge badge-ok">Completed</span>') + '</td><td style="white-space:nowrap;"><button class="btn-sm-primary" title="Receipt for this checkout" onclick="BORcpt.open({group_id:\'' + BO.jsq(s.group_id) + '\'})">🧾</button> ' + (cancelled ? '' : '<button class="btn-sm-danger" title="Cancel this sale &amp; restore stock" onclick="BODash.cancel(\'' + BO.jsq(s.id) + '\',\'' + BO.jsq(s.legacy_id || '') + '\')">🗑️</button>') + '</td></tr>';
      });
