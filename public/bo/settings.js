...
window.BOSet = (function () {
  ...
  /* THE ANDROID APP USED TO BE PUBLISHED FROM HERE, BY HAND.
     It is published by .github/workflows/android-apk.yml now: built, signed, checked, uploaded
     and registered, with the version number climbing on its own. The marketplace already
     carries the download button and the QR code, so this card showed a job nobody had to do
     and offered a second way to do it -- and a hand-typed version code that collided with the
     workflow's run number would have told every phone it was out of date, permanently.
     One way to publish, in one place. The API functions behind it (releases, publishRelease,
